...
=======
			...
			var fid = document.fr.id.value
			window.open("updatePass.jsp?userid=" + fid, "",
		...
		<article>
			...
			<form action="updatePro.jsp" id="join" name="fr" method="post">
				<fieldset>
					...
					<label>User ID</label> <input type="text" name="id" class="id" value="<%=id%>" readonly>
					<br>
					<label>Password</label> <input type="password" name="pass"> <input type="button" name="upPass" value="비밀번호 변경" onclick="updatePass()"> <input type="hidden" name="oriPass" value="<%=mb.getPass()%>">
					<br>
					<label>Name</label> <input type="text" name="name" value="<%=mb.getName()%>">
					<br>
					<label>E-Mail</label> <input type="email" name="email" value="<%=mb.getEmail()%>">
					<br>
					<label>Address</label><input type="text" id="address" name="address" value="<%=mb.getAddress()%>"><input type="button" onclick="execDaumPostcode()" value="주소 검색">
					<br>
					<div id="map" style="width: 300px; height: 300px; margin-top: 10px; display: none"></div>
					<script>
						var mapContainer = document.getElementById('map'), // 지도를 표시할 div
